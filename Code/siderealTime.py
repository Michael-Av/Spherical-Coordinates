import Code.solarNoon
import Code.timeConversions
import datetime
import numpy as np
import math
debug=False
...
def getSiderealTime(latitude, longitude, time):
    ...
    timezone = Code.timeConversions.getTimeZone(latitude, longitude)

    localSolarNoon = Code.solarNoon.calculateSolarNoon(timezone, longitude, time)
    if debug:
